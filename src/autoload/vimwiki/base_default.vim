...
function! vimwiki#base_default#find_next_link() "{{{
  call vimwiki#base#search_word(g:vimwiki_rxWikiLink.'\|'.
        \ g:vimwiki_rxWikiIncl.'\|'.g:vimwiki_rxWeblink, '')
endfunction
" }}}

function! vimwiki#base_default#find_prev_link() "{{{
  call vimwiki#base#search_word(g:vimwiki_rxWikiLink.'\|'.
        \ g:vimwiki_rxWikiIncl.'\|'.g:vimwiki_rxWeblink, 'b')
endfunction
" }}}

function! vimwiki#base_default#follow_link(split, ...) "{{{
  if a:split == "split"
    let cmd = ":split "
  ...
  if a:0 > 0
    ...
    " execute "normal! \n"
    call vimwiki#base_default#normalize_link(0)
  endif

endfunction " }}}

function! vimwiki#base_default#normalize_link(is_visual_mode) "{{{
  if !a:is_visual_mode
